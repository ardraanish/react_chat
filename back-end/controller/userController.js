...
exports.signin = async (req, res) => {
  try {
    ...
    const accessToken = jwt.sign({ user_id: user._id }, process.env.SECRET_KEY, )//{ expiresIn: '15m' }); // Short-lived access token
    const refreshToken = jwt.sign({ user_id: user._id }, process.env.SECRET_KEY, )//{ expiresIn: '7d' }); // Long-lived refresh token
...
exports.sendFriendRequest = async (req, res) => {
  ...
  try {
    ...
    const existingRequest = receiver.friendRequests.find(request => request.senderId.toString() === senderId.toString());
    if (existingRequest) {
